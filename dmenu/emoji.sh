...
# forked from https://github.com/porras/dmenu-emoji/blob/master/dmenu-emoji.sh
...
case "$1" in
  ...
  "copy")
    ...
    if [ ! -z "$input" ]; then
      emoji=${input: -1}
      echo -n "$emoji" | xsel -ib
      command -v notify-send > /dev/null && notify-send -t 500 "$emoji copied!"
    fi
    ;;
  "")
    bash $0 list | dmenu -matching fuzzy -i -p 'Emoji: ' | bash $0 copy
    ;;
esac
